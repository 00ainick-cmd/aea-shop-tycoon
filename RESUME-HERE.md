...
- Git: this game folder is its own dedicated repo (root = this folder) with remote `origin` = https://github.com/00ainick-cmd/aea-shop-tycoon (private). Commit and push here freely (end commit messages with the Claude co-author trailer). The surrounding home directory is a SEPARATE, unrelated git repo, never commit/push that one. The folder also syncs via OneDrive, so either `git clone` on the laptop or just open the synced OneDrive folder.
